...
export type SubstationCategory = "transco" | "lis_ww";

/** One component instance per voltage class and category, so useLayerData is
 * called once per component rather than in a loop - the hook order stays
 * stable even if the voltage list later becomes dynamic. */
export function SubstationLayerGroup({
  ...
  return (
    <IconMarkerLayer
      ...
      tooltip={(s) => s.ss_name ?? `Substation ${s.ss_code}`}
      popup={(s) => (
          ...
          {s.link_ss_photo && (
            ...
            >
              <img
                ...
                loading="lazy"
                // the documents may not be reachable yet; hide rather than
                // leaving a broken-image icon in the popup
                onError={(e) => {
